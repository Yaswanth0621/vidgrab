...
async function extractFromHtml(pageUrl) {
  try {
    console.log(`[HTML Extractor] Fetching: ${pageUrl}`);
    // Fetch the HTML
    const response = await axios.get(pageUrl, {
      headers: getBrowserHeaders(pageUrl),
      timeout: 15000,
      maxRedirects: 5,
      validateStatus: false // Allow us to see 403/404 responses
    });

    if (response.status !== 200) {
      console.warn(`[HTML Extractor] non-200 status for ${pageUrl}: ${response.status}`);
    }
    ...
    // 6. Snapchat Spotlight special
    if (pageUrl.includes("snapchat.com")) {
      console.log("[HTML Extractor] Detected Snapchat, applying deep scan...");
      // Snapchat stores video URL in a large JSON object in a script tag
      const snapMatch = html.match(/"streamingUrl"\s*:\s*"([^"]+)"/);
      if (snapMatch) {
         const url = snapMatch[1].replace(/\\u002f/g, "/");
         found.push({ url, type: detectType(url), source: "snapchat_scanner" });
      }
      const snapMatch2 = html.match(/"mediaUrl"\s*:\s*"([^"]+)"/);
      if (snapMatch2) {
         const url = snapMatch2[1].replace(/\\u002f/g, "/");
         found.push({ url, type: detectType(url), source: "snapchat_scanner" });
      }
    }
    ...
    // 7. data-src, data-video-src attributes
    ...
    // 8. iframe src (for embedded players)
    const iframes = [];
    $("iframe").each((_, el) => {
    ...
    if (unique.length === 0 && iframes.length === 0) {
       console.log(`[HTML Extractor] No videos/iframes found. HTML starts with: ${html.substring(0, 200)}`);
    }

    return { videos: unique, iframes: [...new Set(iframes)], title, thumbnail, description, html };
  } catch (err) {
    console.error(`[HTML Extractor] Error: ${err.message}`);
    return { videos: [], iframes: [], title: "Untitled Video", thumbnail: "", description: "" };
  }
}
